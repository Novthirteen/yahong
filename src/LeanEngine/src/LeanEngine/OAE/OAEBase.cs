...
namespace LeanEngine.OAE
{
    public abstract class OAEBase : IOAE
    {
        ...
        public virtual void ProcessTime(Flow flow)
        {
            ...
            {
                flow.OrderTime = this.GetNextOrderTime(DateTime.Now, windowTimes, weekInterval);

                flow.NextWindowTime = this.GetNextWindowTime(flow.OrderTime, flow.FlowStrategy.LeadTime);
