...
namespace com.Sconit.Utility
{
    ...
    public class Patch
    {
        ...
        public override string ToString()
        {
            ...
            return DiffMatchPatchHelper.UnescapeForEncodeUriCompatability(text.ToString());
        }
    }
    ...
    public static class DiffMatchPatchHelper
    {
        ...
        private static short Diff_EditCost = 4;
        // At what point is no match declared (0.0 = perfection, 1.0 = very loose).
        private static float Match_Threshold = 0.5f;
        // How far to search for a match (0 = exact location, 1000+ = broad match).
        // A match this many characters away from the expected location will add
        // 1.0 to the score (0.0 is a perfect match).
        private static int Match_Distance = 1000;
        // When deleting a large block of text (over ~64 characters), how close
        // do the contents have to be to match the expected contents. (0.0 =
        // perfection, 1.0 = very loose).  Note that Match_Threshold controls
        // how closely the end points of a delete need to match.
        private static float Patch_DeleteThreshold = 0.5f;
        // Chunk size for context length.
        private static short Patch_Margin = 4;

        // The number of bits in an int.
        private static short Match_MaxBits = 32;
         ...
         */
        public static List<Diff> DiffMain(string text1, string text2)
        {
                ...
                switch (diffs[pointer].operation)
                {
                    ...
                    case Operation.EQUAL:
                        ...
                        if (count_delete >= 1 && count_insert >= 1)
                        {
                            ...
                            List<Diff> a = DiffMain(text_delete, text_insert, false, deadline);
         ...
         */
        public static string DiffPrettyHtml(List<Diff> diffs)
        {
        ...
        public static string DiffPrettyHtml(string text1, string text2)
        {
            var diffs = DiffMain(text1, text2);
            StringBuilder html = new StringBuilder();
            foreach (Diff aDiff in diffs)
            {
                aDiff.text = aDiff.text ?? string.Empty;
                string text = aDiff.text.Replace("&", "&amp;").Replace("<", "&lt;")
                  .Replace(">", "&gt;");//.Replace("\n", "&para;<br>");
                switch (aDiff.operation)
                {
         ...
         * @return Source text.
         */
        public static string DiffText1(List<Diff> diffs)
        {
            StringBuilder text = new StringBuilder();
            foreach (Diff aDiff in diffs)
            {
         ...
         */
        public static int MatchMain(string text, string pattern, int loc)
        {
            // Check for null inputs not needed since null can't be passed in C#.
         ...
         */
        private static void PatchAddContext(Patch patch, string text)
        {
            if (text.Length == 0)
            {
                return;
         ...
         */
        public static List<Patch> PatchMake(string text1, string text2)
        {
            // Check for null inputs not needed since null can't be passed in C#.
         ...
         * @return List of Patch objects.
         */
        public static List<Patch> PatchMake(List<Diff> diffs)
        {
            // Check for null inputs not needed since null can't be passed in C#.
         ...
         * @return List of Patch objects.
         */
        public static List<Patch> PatchMake(string text1, List<Diff> diffs)
        {
            // Check for null inputs not needed since null can't be passed in C#.
        ...
        public static Object[] PatchApply(List<Patch> patches, string text)
        {
            ...
            foreach (Patch aPatch in patches)
            {
                ...
                else
                {
                    ...
                    {
                        text2 = text.JavaSubstring(start_loc, Math.Min(end_loc + Match_MaxBits, text.Length));
                    }
                    if (text1 == text2)
                    {
                    ...
                    else
                    {
                        ...
                        if (text1.Length > Match_MaxBits && DiffLevenshtein(diffs) / (float)text1.Length > Patch_DeleteThreshold)
                        {
         ...
         */
        public static string PatchAddPadding(List<Patch> patches)
        {
         ...
         */
        public static void PatchSplitMax(List<Patch> patches)
        {
            ...
            for (int x = 0; x < patches.Count; x++)
            {
                ...
                while (bigpatch.diffs.Count != 0)
                {
                    ...
                    while (bigpatch.diffs.Count != 0 && patch.length1 < patch_size - Patch_Margin)
                    {
