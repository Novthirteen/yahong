...
namespace com.Sconit.Facility.Service.Impl
{
    ...
    public class FacilityMasterMgr : FacilityMasterBaseMgr, IFacilityMasterMgr
    {
        ...
        public void UpdateFacilityMasterAndCreateFacilityTrans(FacilityMaster facilityMaster, FacilityTrans facilityTrans, string status, string userCode, string taskCode)
        {
            ...
            #region 默认取创建日期
            if (facilityTrans.StartDate == null)
            {
                facilityTrans.StartDate = facilityTrans.CreateDate;
            }
            if (facilityTrans.EndDate == null)
            {
                facilityTrans.EndDate = facilityTrans.CreateDate;
            }
            #endregion
                ...
                || facilityTrans.TransType == FacilityConstants.CODE_MASTER_FACILITY_TRANSTYPE_INSPECT_FINISH)
            {
                if (!string.IsNullOrEmpty(taskCode))
                {
                    ...
                    TaskMstr t = taskMasterMgrE.LoadTaskMstr(taskCode);
            ...
            if (facilityMaintainPlanList != null && facilityMaintainPlanList.Count > 0)
            {
                ...
                foreach (FacilityMaintainPlan facilityPlan in facilityMaintainPlanList)
                {
                    ...
                    else
                    {
                        ...
                        task.PlanCompleteDate = facilityPlan.NextMaintainDate.Value.AddHours(facilityPlan.MaintainPlan.Period.HasValue ? (double)facilityPlan.MaintainPlan.Period.Value : 0);
                    }
                    #endregion
        ...
        public IList<AttachmentDetail> GetFacilityStockAttachment(string key)
        {
            return GetAttachment(key, typeof(FacilityStock));
        }
        public int GetFacilityStockAttachmentCount(string key)
        {
            return GetAttachmentCount(key, typeof(FacilityStock));
        }
