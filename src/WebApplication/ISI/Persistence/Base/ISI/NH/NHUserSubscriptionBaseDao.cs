using System;
using System.Collections;
using System.Collections.Generic;
using System.Text;
using Castle.Facilities.NHibernateIntegration;
using NHibernate;
using NHibernate.Type;
using com.Sconit.ISI.Entity;
using com.Sconit.Persistence;

//TODO: Add other using statmens here.

namespace com.Sconit.ISI.Persistence.NH
{
    public class NHUserSubscriptionBaseDao : NHDaoBase, IUserSubscriptionBaseDao
    {
        public NHUserSubscriptionBaseDao(ISessionManager sessionManager)
            : base(sessionManager)
        {
        }

        #region Method Created By CodeSmith

        public virtual void CreateUserSubscription(UserSubscription entity)
        {
            Create(entity);
        }

        public virtual IList<UserSubscription> GetAllUserSubscription()
        {
            return FindAll<UserSubscription>();
        }

        public virtual UserSubscription LoadUserSubscription(Int32 id)
        {
            return FindById<UserSubscription>(id);
        }

        public virtual void UpdateUserSubscription(UserSubscription entity)
        {
            Update(entity);
        }

        public virtual void DeleteUserSubscription(Int32 id)
        {
            string hql = @"from UserSubscription entity where entity.Id = ?";
            Delete(hql, new object[] { id }, new IType[] { NHibernateUtil.Int32 });
        }

        public virtual void DeleteUserSubscription(UserSubscription entity)
        {
            Delete(entity);
        }

        public virtual void DeleteUserSubscription(IList<Int32> pkList)
        {
            StringBuilder hql = new StringBuilder();
            hql.Append("from UserSubscription entity where entity.Id in (");
            hql.Append(pkList[0]);
            for (int i = 1; i < pkList.Count; i++)
            {
                hql.Append(",");
                hql.Append(pkList[i]);
            }
            hql.Append(")");

            Delete(hql.ToString());
        }

        public virtual void DeleteUserSubscription(IList<UserSubscription> entityList)
        {
            IList<Int32> pkList = new List<Int32>();
            foreach (UserSubscription entity in entityList)
            {
                pkList.Add(entity.Id);
            }

            DeleteUserSubscription(pkList);
        }


        #endregion Method Created By CodeSmith
    }
}
