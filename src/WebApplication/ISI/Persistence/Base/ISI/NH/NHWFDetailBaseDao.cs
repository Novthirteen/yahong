using System;
using System.Collections;
using System.Collections.Generic;
using System.Text;
using Castle.Facilities.NHibernateIntegration;
using NHibernate;
using NHibernate.Type;
using com.Sconit.ISI.Entity;
using com.Sconit.Persistence;

//TODO: Add other using statmens here.

namespace com.Sconit.ISI.Persistence.NH
{
    public class NHWFDetailBaseDao : NHDaoBase, IWFDetailBaseDao
    {
        public NHWFDetailBaseDao(ISessionManager sessionManager)
            : base(sessionManager)
        {
        }

        #region Method Created By CodeSmith

        public virtual void CreateWFDetail(WFDetail entity)
        {
            Create(entity);
        }

        public virtual IList<WFDetail> GetAllWFDetail()
        {
            return FindAll<WFDetail>();
        }

        public virtual WFDetail LoadWFDetail(Int32 id)
        {
            return FindById<WFDetail>(id);
        }

        public virtual void UpdateWFDetail(WFDetail entity)
        {
            Update(entity);
        }

        public virtual void DeleteWFDetail(Int32 id)
        {
            string hql = @"from WFDetail entity where entity.Id = ?";
            Delete(hql, new object[] { id }, new IType[] { NHibernateUtil.Int32 });
        }

        public virtual void DeleteWFDetail(WFDetail entity)
        {
            Delete(entity);
        }

        public virtual void DeleteWFDetail(IList<Int32> pkList)
        {
            StringBuilder hql = new StringBuilder();
            hql.Append("from WFDetail entity where entity.Id in (");
            hql.Append(pkList[0]);
            for (int i = 1; i < pkList.Count; i++)
            {
                hql.Append(",");
                hql.Append(pkList[i]);
            }
            hql.Append(")");

            Delete(hql.ToString());
        }

        public virtual void DeleteWFDetail(IList<WFDetail> entityList)
        {
            IList<Int32> pkList = new List<Int32>();
            foreach (WFDetail entity in entityList)
            {
                pkList.Add(entity.Id);
            }

            DeleteWFDetail(pkList);
        }


        #endregion Method Created By CodeSmith
    }
}
