...
{
    [Transactional]
    public class RepSCBTMgr : ReportBaseMgr
    {
        ...
        public override bool FillValues(String templateFileName, IList<object> list)
        {
            try
            {
                ...
                this.SetRowCell(pageIndex, rowIndex, 3, task.CostCenter);
                ...
                if (task.IsApply)
                {
                    ...
                    {
                        this.SetRowCell(pageIndex, 2, 4, apply.Desc1 + "：");
                        this.SetRowCell(pageIndex, 2, 5, apply.Qty.Value.ToString("0.########"));
                    ...
                    if (apply != null && apply.Qty.HasValue)
                    {
                        this.SetRowCell(pageIndex, 3, 0, apply.Desc1 + "：");
                        this.SetRowCell(pageIndex, 3, 1, apply.GetValue());
                        taskApplyList.Remove(apply);
                    }
                }

                if (task.Voucher.HasValue && task.Voucher.Value != 0)
                {
                    this.SetRowCell(pageIndex, 2, 5, task.Voucher.Value.ToString());
                }

                if (task.Amount.HasValue && task.Amount.Value != 0)
                {
                    this.SetRowCell(pageIndex, 3, 1, ISIUtil.ToMoneyString(task.Amount));
                }
                ...
                this.SetRowCell(pageIndex, 5, 0, desc.ToString());
                ...
                if (task.IsWF)
                {
                    ...
                    if (traceViewList != null && traceViewList.Count > 0)
                    {
                        ...
                        this.SetRowCell(pageIndex, 7, 0, desc.ToString());
                    }
                }
...
{
    [Transactional]
    public partial class RepSCBTMgrE : com.Sconit.ISI.Service.Report.Impl.RepSCBTMgr, IReportBaseMgrE
    {
