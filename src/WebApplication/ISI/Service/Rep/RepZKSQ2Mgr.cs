...
{
    [Transactional]
    public class RepZKSQ2Mgr : ReportBaseMgr
    {
        ...
        public override bool FillValues(String templateFileName, IList<object> list)
        {
            try
            {
                ...
                if (task.IsApply)
                {
                    ...
                    //支付方式：
                    var apply = taskApplyList.Where(ta => ta.UOMDesc1 == "公司").FirstOrDefault();
                    if (apply != null)
                    {
                        this.SetRowCell(pageIndex, 1, 0, apply.UOMDesc1 + "：");
                        this.SetRowCell(pageIndex, 1, 1, apply.Desc1);
...
{
    [Transactional]
    public partial class RepZKSQ2MgrE : com.Sconit.ISI.Service.Report.Impl.RepZKSQ2Mgr, IReportBaseMgrE
    {
