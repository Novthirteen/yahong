            ...
            try
            {
                ...
                foreach (InspectResult inspectResult in inspectResultList)
                {
                    if (inspectResult.RejectedQty > 0)
                    {
                        ...
                        this.SetRowCell(pageIndex, rowIndex, 4, string.Empty); //inspectOrderDetail.DefectClassification
                        //处理方法             Disposition 
                        if (inspectResult.InspectOrderDetail.Disposition != null && inspectResult.InspectOrderDetail.Disposition != string.Empty)
                        {
                            CodeMaster codeMaster = codeMasterMgrE.GetCachedCodeMaster(BusinessConstants.CODE_MASTER_INSPECT_DISPOSITION, inspectResult.InspectOrderDetail.Disposition);
                        ...
                        //起末库位	
                        this.SetRowCell(pageIndex, rowIndex, 6, inspectResult.InspectOrderDetail.LocationFrom.Code);
                        //总成零件号	
                        this.SetRowCell(pageIndex, rowIndex, 7, inspectResult.InspectOrderDetail.FinishGoods == null ? string.Empty : inspectResult.InspectOrderDetail.FinishGoods.Code);
