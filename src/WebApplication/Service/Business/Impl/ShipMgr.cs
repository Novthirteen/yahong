...
namespace com.Sconit.Service.Business.Impl
{
    public class ShipMgr : AbstractBusinessMgr
    {
        ...
        private Resolver ShipOrder(Resolver resolver)
        {
            ...
            if (orderHead.IsAutoReceive)
            {
                IList<Receipt> receipts = receiptMgrE.GetReceipts(inProcessLocation.IpNo);
                if (receipts != null && receipts.Count > 0)
                {
                    resolver.Result = languageMgrE.TranslateMessage("Common.Business.RecNoIs", resolver.UserCode, receipts.LastOrDefault().ReceiptNo);
                    if (orderHead.NeedPrintReceipt)
                    {
                        //如果自动收货且打印收货单,则不打印ASN,只打印ASN对应的最后一张的收货单
                        executeMgrE.PrintReceipt(resolver, receipts.LastOrDefault());
                    }
                }
            }
            else
            {
                if (orderHead.NeedPrintAsn)
                {
