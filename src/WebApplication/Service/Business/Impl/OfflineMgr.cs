            ...
            else if (resolver.CodePrefix == BusinessConstants.BARCODE_HEAD_FLOW)
            {
                try
                {
                    ...
                    else
                    {
                        ...
                        IList<string> orderNoList = (from t in orderHeadList
                                    select t.OrderNo).ToList();
                        IList<OrderLocationTransaction> orderLocationTransactionList = orderLocationTransactionMgrE.GetOrderLocationTransaction(orderNoList, BusinessConstants.IO_TYPE_IN);
                        resolver.Transformers = TransformerHelper.ConvertOrderLocationTransactionToTransformers(orderLocationTransactionList);
