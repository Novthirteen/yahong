...
namespace com.Sconit.Service.Distribution.Impl
{
    ...
    public class InProcessLocationMgr : InProcessLocationBaseMgr, IInProcessLocationMgr
    {
        ...
        public override void CreateInProcessLocation(InProcessLocation entity)
        {
            if (entity.OrderType != BusinessConstants.CODE_MASTER_ORDER_TYPE_VALUE_PRODUCTION)
            {
                base.CreateInProcessLocation(entity);
            }
        }
        ...
        public void CreateInProcessLocation(InProcessLocation inProcessLocation, User user, string type)
        {
            ...
            string ExternalOrderNo = string.Empty;
            string ReferenceOrderNo = string.Empty;
            DateTime? SettleTime = null;
            ...
            foreach (InProcessLocationDetail inProcessLocationDetail in inProcessLocation.InProcessLocationDetails)
            {
                ...
                if (orderHead.ReferenceOrderNo != null && orderHead.ReferenceOrderNo.Trim() != string.Empty)
                {
                    bool isHas = false;
                    if (ReferenceOrderNo != null)
                    {
                        isHas = ReferenceOrderNo.Contains(orderHead.ReferenceOrderNo);
                    }
                    if (!isHas)
                    {
                        ReferenceOrderNo += orderHead.ReferenceOrderNo + " ";
                    }
                }
                if (orderHead.ExternalOrderNo != null && orderHead.ExternalOrderNo.Trim() != string.Empty)
                {
                    bool isHas = false;
                    if (ExternalOrderNo != null)
                    {
                        isHas = ExternalOrderNo.Contains(orderHead.ExternalOrderNo);
                    }
                    if (!isHas)
                    {
                        ExternalOrderNo += orderHead.ExternalOrderNo + " ";
                    }
                }
                SettleTime = orderHead.SettleTime;
            }
            ...
            ReferenceOrderNo = ReferenceOrderNo.Trim();
            if (ReferenceOrderNo != null && ReferenceOrderNo != string.Empty)
            {
                inProcessLocation.ReferenceOrderNo = ReferenceOrderNo;
            }
            ExternalOrderNo = ExternalOrderNo.Trim();
            if (ExternalOrderNo != null && ExternalOrderNo != string.Empty)
            {
                inProcessLocation.ExternalOrderNo = ExternalOrderNo;
            }
            inProcessLocation.DepartTime = dateTimeNow;
            Flow flow = this.flowMgr.CheckAndLoadFlow(flowCode);
            if (isEmergency)
            {
                inProcessLocation.ArriveTime = inProcessLocation.DepartTime.Value.AddHours(Convert.ToDouble(flow.EmTime.HasValue ? flow.EmTime.Value : 0));
            }
            else
            {
                inProcessLocation.ArriveTime = inProcessLocation.DepartTime.Value.AddHours(Convert.ToDouble(flow.LeadTime.HasValue ? flow.LeadTime.Value : 0));
            }
            ...
            inProcessLocation.Flow = flowCode;
            inProcessLocation.SettleTime = SettleTime;
            ...
            if (targetInProcessLocationDetailList != null && targetInProcessLocationDetailList.Count > 0)
            {
                ...
                if (type == BusinessConstants.CODE_MASTER_INPROCESS_LOCATION_TYPE_VALUE_NORMAL)
                {
                        ...
                        else
                        {
                            ...
                            if (orderLocationTransaction.Location != null)
                            {
                                ...
                                if (orderHead.Type == BusinessConstants.CODE_MASTER_ORDER_TYPE_VALUE_PRODUCTION
                                    ...
                                    && inProcessLocationDetail.Qty > 0 && orderLocationTransaction.BomDetail != null)
                                {
                                    itemDiscontinueList = this.itemDiscontinueMgr.GetItemDiscontinue(orderLocationTransaction.Item, orderLocationTransaction.BomDetail.Bom, dateTimeNow);
                                }
                                #endregion
                                ...
                                if (orderDetail.NeedRejectInspection && orderHead.NeedRejectInspection
                                    && (orderHead.SubType == BusinessConstants.CODE_MASTER_ORDER_SUB_TYPE_VALUE_RTN || orderHead.SubType == BusinessConstants.CODE_MASTER_ORDER_SUB_TYPE_VALUE_RWO)
                    ...
                    if (inspectLocationLotDetailList.Count > 0)
                    {
                        ...
                        this.inspectOrderMgrE.CreateInspectOrder(inspectLocationLotDetailList, inspectLocation, rejectLocation, user, inProcessLocation.IpNo, null, false);
                    }
                    #endregion
                }
                else
        ...
        public void ResolveInPorcessLocationNormal(InProcessLocation inProcessLocation, string grGapTo, User user)
        {
            ...
            foreach (InProcessLocationDetail nmlInProcessLocationDetail in nmlInProcessLocation.InProcessLocationDetails)
            {
                if (grGapTo == BusinessConstants.CODE_MASTER_GR_GAP_TO_GI)
                {
                    ...
                    #endregion
                    //更新订单数量
                    this.orderDetailMgrE.RecordOrderShipQty(orderLocationTransaction, nmlInProcessLocationDetail, false);
