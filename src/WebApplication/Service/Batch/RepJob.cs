...
{
    [Transactional]
    public abstract class RepJob : IJob
    {
        ...
        private ICellStyle cellStyleCenter;
        ...
        public virtual void SendEmail(string key, string permissionCode, string desc, DateTime? startDate, DateTime? endDate)
        {
            try
            {
                ...
                IList<EntityPreference> entityPreferenceList = entityPreferenceMgrE.GetEntityPreferenceOrderBySeq(new string[]{BusinessConstants.ENTITY_PREFERENCE_CODE_COMPANYNAME,
                                                    BusinessConstants.ENTITY_PREFERENCE_WEBADDRESS,BusinessConstants.ENTITY_PREFERENCE_CODE_DEBUGMODE,BusinessConstants.ENTITY_PREFERENCE_CODE_DEBUGMODEEMAIL});
                ...
                {
                    mailList = entityPreferenceList.Where(e => e.Code == BusinessConstants.ENTITY_PREFERENCE_CODE_DEBUGMODEEMAIL).SingleOrDefault().Value;
                }
                ...
                string webAddress = entityPreferenceList.Where(e => e.Code == BusinessConstants.ENTITY_PREFERENCE_WEBADDRESS).SingleOrDefault().Value;

                string file = GenerateReport(key, desc, entityPreferenceList, startDate, endDate);
                ...
                if (!string.IsNullOrEmpty(file))
                {
                    ...
                    this.AppendTestText(companyName, content, separator);
                    ...
                    string emailFrom = entityPreferenceList.Where(e => e.Code == BusinessConstants.ENTITY_PREFERENCE_CODE_SMTPEMAILADDR).SingleOrDefault().Value;
                    string userMail = emailFrom;
                    string SMTPEmailHost = entityPreferenceList.Where(e => e.Code == BusinessConstants.ENTITY_PREFERENCE_CODE_SMTPEMAILHOST).SingleOrDefault().Value;
                    string SMTPEmailPasswd = entityPreferenceList.Where(e => e.Code == BusinessConstants.ENTITY_PREFERENCE_CODE_SMTPEMAILPASSWD).SingleOrDefault().Value;
                    bool result = SMTPHelper.SendSMTPEMail(companyName + "-" + key, content.ToString(), emailFrom, mailList, SMTPEmailHost, SMTPEmailPasswd, userMail, files);
                    if (!result)
                    {
                        log.Error("发送失败");
                    }
        ...
        public void AppendTestText(string companyName, StringBuilder content, string separator)
        {
            bool isTest = false;
        ...
        private string GenerateReport(string key, string desc, IList<EntityPreference> entityPreferenceList, DateTime? startDate, DateTime? endDate)
        {
            string companyName = entityPreferenceList.Where(e => e.Code == BusinessConstants.ENTITY_PREFERENCE_CODE_COMPANYNAME).SingleOrDefault().Value;
            ...
            return XlsHelper.WriteToLocalZip(companyName + "-" + key + ".xls", workbook);
        }
        ...
        protected virtual bool ProcessOrder(string orderType, string subType, string flowType, IWorkbook workbook, DateTime? startDate, DateTime? endDate)
        {
            try
            {
                ...
                orderMstrSql.Append(@" join FlowMstr f on o.Flow = f.Code and ");
                ...
                orderMstrSql.Append(@" join Party p1 on o.PartyFrom=p1.Code ");
                orderMstrSql.Append(@" join Party p2 on o.PartyTo=p2.Code ");
                orderMstrSql.Append(@" join PartyAddr pa1 on o.ShipFrom=pa1.Code and pa1.AddrType= 'ShipAddr' ");
                orderMstrSql.Append(@" join PartyAddr pa2 on o.ShipTo=pa2.Code and pa2.AddrType= 'ShipAddr' ");

                orderMstrSql.Append(@" join CodeMstr c1 on o.Status=c1.CodeValue and c1.Code = '" + BusinessConstants.CODE_MASTER_STATUS + "'");
                orderMstrSql.Append(@" join CodeMstr c2 on o.SubType=c2.CodeValue and c2.Code = '" + BusinessConstants.CODE_MASTER_ORDER_SUB_TYPE + "'");
                orderMstrSql.Append(@" join CodeMstr c3 on o.Priority=c3.CodeValue and c3.Code = '" + BusinessConstants.CODE_MASTER_ORDER_PRIORITY + "'");
                orderMstrSql.Append(@" join CodeMstr c5 on o.Type=c5.CodeValue and c5.Code = '" + BusinessConstants.CODE_MASTER_ORDER_TYPE + "'");
                orderMstrSql.Append(@" left join Location l1 on o.LocFrom=l1.Code ");
                orderMstrSql.Append(@" left join Location l2 on o.LocTo=l2.Code ");
                orderMstrSql.Append(@" left join CodeMstr c4 on o.BillSettleTerm=c4.CodeValue and c4.Code = '" + BusinessConstants.CODE_MASTER_BILL_SETTLE_TERM + "'");
                orderMstrSql.Append(@" left join Currency c on o.Currency=c.Code ");
                ...
                if (orderMstrList != null && orderMstrList.Count > 0)
                {
                    ...
                    orderDetSql.Append(@"        l1.Name+'['+l1.Code+']' LocFrom,l2.Name+'['+l2.Code+']' LocTo,i.NumField3 * 0.8 Price,i.NumField3 * 0.8 * d.OrderQty Amount ");//,d.UnitPriceAfterDiscount
                    orderDetSql.Append(@" from OrderDet d ");
                    orderDetSql.Append(@" join OrderMstr o on o.OrderNo = d.OrderNo ");
                    orderDetSql.Append(@" join Item i on i.Code = d.Item ");
                    orderDetSql.Append(@" left join Location l1 on o.LocFrom=l1.Code ");
                    orderDetSql.Append(@" left join Location l2 on o.LocTo=l2.Code ");
        ...
        private bool ProcessOrder(string orderType, string subType, IWorkbook workbook, IList<OrderMstr> orderMstrList, IList<OrderDet> orderDetList, DateTime startDate, DateTime endDate)
        {
            ...
            {
                ProcessOrder(orderType, BusinessConstants.CODE_MASTER_ORDER_SUB_TYPE_VALUE_NML, string.Empty, "逾期" + type + "单", workbook, nmlOrderMstrList, orderDetList, endDate);
            }
        ...
        private bool ProcessOrder(string orderType, string subType, string dateTimeType, string type, IWorkbook workbook, IList<OrderMstr> orderMstrList, IList<OrderDet> orderDetList, DateTime endDate)
        {
            try
            {
                ...
                sheet.SetColumnWidth(16, 18 * 256);//路线
                sheet.SetColumnWidth(17, 10 * 256);//结算方式
                sheet.SetColumnWidth(18, 8 * 256);//币种
                int rownum = 0;
                int column = 0;
        ...
        protected virtual bool ProcessGap(string orderType, IWorkbook workbook, DateTime? startDate, DateTime? endDate)
        {
            try
            {
                ...
                ipDetSql.Append(@"       d.HuId,d.LotNo,d.Qty, isnull(d.Uom,od.Uom) Uom,isnull(d.UC,od.UC) UC,d.RecQty,d.Qty - d.RecQty DiffQty, ");//d.CustomerItemCode,
                ...
                ipDetSql.Append(@"join IpDet d on ip.ipno=d.ipno and ip.OrderType='" + orderType + "' ");
                ipDetSql.Append(@"join OrderLocTrans olt on d.OrderLocTransId =olt.Id ");
                ipDetSql.Append(@"join OrderDet od on olt.OrderDetId=od.Id  ");
                ipDetSql.Append(@"join Item i on olt.Item=i.Code ");
                ipDetSql.Append(@"join Party p1 on p1.Code=ip.PartyFrom  ");
                ipDetSql.Append(@"join Party p2 on p2.Code=ip.PartyTo ");
                ipDetSql.Append(@"join PartyAddr pa1 on ip.ShipFrom=pa1.Code and pa1.AddrType= 'ShipAddr' ");
                ipDetSql.Append(@"join PartyAddr pa2 on ip.ShipTo=pa2.Code and pa2.AddrType= 'ShipAddr' ");
                ipDetSql.Append(@"join ACC_User u on u.USR_Code=ip.CreateUser ");
        ...
        protected virtual bool ProcessASN(string orderType, IWorkbook workbook, DateTime? startDate, DateTime? endDate)
        {
            try
            {
                ...
                if (ipList != null && ipList.Count > 0)
                {
                    ...
                    StringBuilder ipDetSql = new StringBuilder();
                    ipDetSql.Append(@"select d.IpNo,i.Desc1+'['+ i.Code+']' Item,d.RefItemCode, ");
                    ipDetSql.Append(@"       d.HuId,d.LotNo,d.Qty, isnull(d.Uom,od.Uom) Uom,isnull(d.UC,od.UC) UC  ");//d.CustomerItemCode,
                    ipDetSql.Append(@"from IpMstr ip ");
                    ipDetSql.Append(@"join IpDet d on ip.IpNo = d.IpNo and ip.OrderType='" + orderType + "' ");
                    ipDetSql.Append(@"join OrderLocTrans olt on d.OrderLocTransId =olt.Id ");
                    ipDetSql.Append(@"join OrderDet od on olt.OrderDetId=od.Id  ");
        ...
        #region 账龄
        protected bool ProcessSOBillAging(IWorkbook workbook)
        {
            return ProcessBillAging(BusinessConstants.BILL_TRANS_TYPE_SO, workbook);
        }

        protected bool ProcessPOBillAging(IWorkbook workbook)
        {
            return ProcessBillAging(BusinessConstants.BILL_TRANS_TYPE_PO, workbook);
        }

        [Transaction(TransactionMode.Unspecified)]
        protected virtual bool ProcessBillAging(string transType, IWorkbook workbook)
        {
            try
            {
                #region 获取头数据
                ...
                billSql.Append(@"        Qty10, i.NumField3 * 0.8 * Qty10 Amount10,Qty11, i.NumField3 * 0.8 * Qty11 Amount11,");
                //billSql.Append(@"        Qty1, i.NumField3 * 0.8 * Qty1 Amount1, ");
                billSql.Append(@"        Qty2, i.NumField3 * 0.8 * Qty2 Amount2, ");
                billSql.Append(@"        Qty3, i.NumField3 * 0.8 * Qty3 Amount3, Qty4, i.NumField3 * 0.8 * Qty4 Amount4, ");
                billSql.Append(@"        Qty5, i.NumField3 * 0.8 * Qty5 Amount5, Qty6, i.NumField3 * 0.8 * Qty6 Amount6, ");
                billSql.Append(@"        Qty7, i.NumField3 * 0.8 * Qty7 Amount7, Qty8, i.NumField3 * 0.8 * Qty8 Amount8, ");
                billSql.Append(@"        Qty9, i.NumField3 * 0.8 * Qty9 Amount9, i.NumField3 * 0.8 Price ");
                billSql.Append(@" from BillAgingView b ");
                billSql.Append(@" join Item i on b.Item =i.Code ");
                billSql.Append(@" join PartyAddr a on b.BillAddr = a.Code and a.AddrType='BillAddr' ");
                billSql.Append(@" join Party p on a.PartyCode = p.Code ");
                billSql.Append(@" where TransType='" + transType + "' ");
                billSql.Append(@" order by p.Code ASC,i.Code ASC,b.Uom ASC,b.UC ASC ");
                DataSet orderMstrDS = sqlHelperMgrE.GetDatasetBySql(billSql.ToString());
                ...
                if (billList != null && billList.Count > 0)
                {
                    ...
                    detSql.Append(@"       i.NumField3 * 0.8 Price,i.NumField3 * 0.8 * Qty Amount  ");
                    detSql.Append(@"from ActBillView a ");
                    detSql.Append(@"join Item i on a.Item=i.Code ");
                    detSql.Append(@"where TransType='" + transType + "' ");
                    detSql.Append(@"order by a.BillAddr ASC,i.Code ASC,a.Uom ASC,a.UC ASC,a.EffDate ASC ");
                    DataSet orderDetDS = sqlHelperMgrE.GetDatasetBySql(detSql.ToString());
                    var actBillList = IListHelper.DataTableToList<ActBill>(orderDetDS.Tables[0]);
                    #endregion

                    return ProcessBillAging(transType, workbook, billList, actBillList);
                }
            }
            catch (Exception e)
            {
        ...
        private bool ProcessBillAging(string transType, IWorkbook workbook, IList<BillAging> billList, IList<ActBill> actBillList)
        {
            try
            {
                #region 初始化Sheet

                ISheet sheet = workbook.CreateSheet("账龄");
        ...
        #region  生产异常
        [Transaction(TransactionMode.Unspecified)]
        protected virtual bool ProcessWO(IWorkbook workbook, DateTime? startDate, DateTime? endDate)
        {
            try
            {
                #region 获取订单头数据
                ...
                orderMstrSql.Append(@" from OrderMstr o ");
                orderMstrSql.Append(@" join FlowMstr f on o.Flow = f.Code and f.Type = '" + BusinessConstants.CODE_MASTER_ORDER_TYPE_VALUE_PRODUCTION + "' ");
                orderMstrSql.Append(@" join Party p1 on o.PartyFrom=p1.Code ");
                //orderMstrSql.Append(@" join Party p2 on o.PartyTo=p2.Code ");
                //orderMstrSql.Append(@" join PartyAddr pa1 on o.ShipFrom=pa1.Code and pa1.AddrType= 'ShipAddr' ");
                //orderMstrSql.Append(@" join PartyAddr pa2 on o.ShipTo=pa2.Code and pa2.AddrType= 'ShipAddr' ");
                ...
                orderMstrSql.Append(@" join CodeMstr c3 on o.Priority=c3.CodeValue and c3.Code = '" + BusinessConstants.CODE_MASTER_ORDER_PRIORITY + "'");
                //orderMstrSql.Append(@" join CodeMstr c5 on o.Type=c5.CodeValue and c5.Code = '" + BusinessConstants.CODE_MASTER_ORDER_TYPE + "'");
                orderMstrSql.Append(@" join Location l1 on o.LocFrom=l1.Code ");
                orderMstrSql.Append(@" join Location l2 on o.LocTo=l2.Code ");
                orderMstrSql.Append(@" join ACC_User u1 on u1.USR_Code = o.CreateUser ");
                orderMstrSql.Append(@" left join ACC_User u2 on u2.USR_Code = o.CancelUser ");
                //orderMstrSql.Append(@" left join CodeMstr c4 on o.BillSettleTerm=c4.CodeValue and c4.Code = '" + BusinessConstants.CODE_MASTER_BILL_SETTLE_TERM + "'");
                //orderMstrSql.Append(@" left join Currency c on o.Currency=c.Code ");
                orderMstrSql.Append(@" where o.Type = '" + BusinessConstants.CODE_MASTER_ORDER_TYPE_VALUE_PRODUCTION + "'  ");
                ...
                orderMstrSql.Append(@" and o.CancelDate >='" + startDate.Value.AddDays(-1) + "' and o.CancelDate < '" + startDate.Value + "' )) ");
                ...
                if (orderMstrList != null && orderMstrList.Count > 0)
                {
                    ...
                    orderDetSql.Append(@" left join Location l1 on o.LocFrom=l1.Code ");
                    orderDetSql.Append(@" left join Location l2 on o.LocTo=l2.Code ");
                    orderDetSql.Append(@" where o.Type = '" + BusinessConstants.CODE_MASTER_ORDER_TYPE_VALUE_PRODUCTION + "'  ");
                    ...
                    orderDetSql.Append(@" and o.CancelDate >='" + startDate.Value.AddDays(-1) + "' and o.CancelDate < '" + startDate.Value + "' )) ");
                    ...
                    {
                        ProcessWO(SHEET_NAME_NO_CLOSE, workbook, noCloseOrderList, orderDetList);
                    }
                    var cancelOrderList = orderMstrList.Where(o => o.Status == "取消").ToList();
                    if (cancelOrderList != null && cancelOrderList.Count > 0)
                    {
                        ProcessWO(SHEET_NAME_CANCEL, workbook, cancelOrderList, orderDetList);
                    }
        ...
        private void ProcessWO(string sheetName, IWorkbook workbook, IList<OrderMstr> orderMstrList, IList<OrderDet> orderDetList)
        {

            #region 初始化Sheet

            ISheet woSheet = workbook.CreateSheet(sheetName);
            ...
            //woSheet.SetColumnWidth(13, 10 * 256);
            int rownum = 0;
            int colnum = 0;
            ...
            if (sheetName == SHEET_NAME_NO_CLOSE)
            {
        ...
        [Transaction(TransactionMode.Unspecified)]
        protected virtual bool ProcessUnpCyc(IWorkbook workbook, DateTime? startDate, DateTime? endDate)
        {
            try
            {
                /*
                ...
                */
                ...
                #region 初始化Sheet
                bool rtn = false;
                ISheet sheet1 = workbook.CreateSheet("库存调整");
                ...
                cycResultSql.Append(@" r.ProcessDate,r.ProcessUserNm+'['+r.ProcessUser+']' ProcessUser,i.NumField3 * 0.8 Price,i.NumField3 * 0.8 * r.DiffQty Amount ");
                cycResultSql.Append(@" from CycleCountResult r ");
                cycResultSql.Append(@" join CycleCountMstr m on m.Code=r.OrderNo ");
                cycResultSql.Append(@" join Item i on i.Code=r.Item ");
                cycResultSql.Append(@" left join Location l on l.Code=r.Location ");
                ...
                if (cycResultList != null && cycResultList.Count() > 0)
                {
                    ...
                    cycMstrSql.Append(@" join ACC_User u on u.USR_Code = m.CompleteUser ");
                    cycMstrSql.Append(@" join CodeMstr c1 on c1.Code = '" + BusinessConstants.CODE_MASTER_PHYCNT_TYPE + "' and c1.CodeValue=m.Type ");
                    cycMstrSql.Append(@" join CodeMstr c2 on c2.Code = '" + BusinessConstants.CODE_MASTER_STATUS + "' and c2.CodeValue=m.Status ");
                    cycMstrSql.Append(@" left join CodeMstr c3 on c3.Code = '" + BusinessConstants.CODE_MASTER_PHYCNT_GROUPBY + "' and c3.CodeValue=m.PhyCntGroupBy ");
                    cycMstrSql.Append(@" left join Location l on l.Code=m.Location ");
        ...
        protected virtual bool ProcessUnpIndustrialWaste(IWorkbook workbook, DateTime? startDate, DateTime? endDate)
        {
            try
            {
                ...
                {
                    //计划外出入库数据
                    var miscOrderList = GetMisOrder(startDate.Value, endDate.Value, @" and m.Reason= '" + BusinessConstants.CODE_MASTER_STOCK_OUT_REASON_VALUE_REASON1 + "'");

                    if (miscOrderList != null && miscOrderList.Count > 0)
                    {
                        var miscOrderDetList = GetMisOrderDet(startDate.Value, endDate.Value, @" and m.Reason= '" + BusinessConstants.CODE_MASTER_STOCK_OUT_REASON_VALUE_REASON1 + "'");
                        if (miscOrderDetList != null && miscOrderDetList.Count > 0)
                        {
                            #region 初始化Sheet
                            bool rtn = false;
                            ISheet sheet1 = workbook.CreateSheet("工废");
        ...
        protected virtual IList<MiscOrder> GetMisOrder(DateTime startDate, DateTime endDate, string where)
        {
            try
            {
                ...
                miscOrderSql.Append(@" join Location l on m.Location = l.Code  ");
                miscOrderSql.Append(@" join ACC_User u on m.CreateUser = u.USR_Code  ");
                miscOrderSql.Append(@" join CodeMstr c1 on c1.Code in ('" + BusinessConstants.CODE_MASTER_STOCK_OUT_REASON + "','" + BusinessConstants.CODE_MASTER_STOCK_IN_REASON + "') and c1.CodeValue=m.Reason ");
                miscOrderSql.Append(@" join CodeMstr c2 on c2.Code = '" + BusinessConstants.CODE_MASTER_MISC_ORDER_TYPE + "' and c2.CodeValue=m.Type ");
        ...
        protected virtual IList<MiscOrderDetail> GetMisOrderDet(DateTime startDate, DateTime endDate, string where)
        {
            try
            {
                ...
                miscOrderDetSql.Append(@" select m.Type,d.Id,d.OrderNo,d.Item+'['+i.Desc1+']' Item,d.HuId,d.LotNo,d.Qty,i.NumField3 * 0.8 Price,i.NumField3 * 0.8 * d.Qty Amount ");
                miscOrderDetSql.Append(@" from MiscOrderDet d ");
                miscOrderDetSql.Append(@" join MiscOrderMstr m on d.OrderNo = m.OrderNo ");
                miscOrderDetSql.Append(@" join Item i on i.Code = d.Item ");
    ...
    class ReceiptDet
    {
        ...
        public Decimal PassRate
        {
            get
            {
                decimal totalQty = this.RecQty + this.RejQty + this.ScrapQty;
                if (totalQty != 0)
                {
    ...
    class BillAging
    {
        ...
        public Decimal Qty
        {
            get
            {
                return Qty2 + Qty4 + Qty5 + Qty6 + Qty7 + Qty8 + Qty9 + Qty10 + Qty11;
            }
        }
        ...
        public Decimal? Amount
        {
            get
            {
                return Amount2 + Amount4 + Amount5 + Amount6 + Amount7 + Amount8 + Amount9 + Amount10 + Amount11;
            }
        }
    }
