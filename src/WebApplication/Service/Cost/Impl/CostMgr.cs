...
namespace com.Sconit.Service.Cost.Impl
{
    ...
    public class CostMgr : SessionBase, ICostMgr
    {
        ...
        private static log4net.ILog log = log4net.LogManager.GetLogger("Log.Cost");
        ...
        public void RecordProductionCostTransaction(Receipt receipt, User user)
        {
            ...
            foreach (var consume in consumeQty)
            {
                ...
                costTransaction.ItemCategory = consume.ReceiptDet.OrderLocationTransaction.OrderDetail.Item.ItemCategory.Code;
            ...
            foreach (var rec in recList)
            {
                    ...
                    for (int i = 0; i < rec.ItemList.Count(); i++)
                    {
                        ...
                        foreach (var ccOp in ccOperationGroup)
                        {
                            foreach (var operation in ccOp.Operations)
                            {
                                ...
                                {
                                    laborHour += operation.RunTime.Value / 60 * receivedQty.Value;
                                    if (operation.WorkCenter.LaborRate.HasValue)
                                    {
                                        laborCost += operation.RunTime.Value / 60 * operation.WorkCenter.LaborRate.Value * receivedQty.Value;
                                    }
                                }
                                #endregion
                                ...
                                {
                                    laborCost += operation.RunTime.Value / 60 * operation.WorkCenter.LaborBurdenRate.Value * receivedQty.Value;
                                }
                                #endregion
                                ...
                                {
                                    laborCost += operation.RunTime.Value / 60 * operation.WorkCenter.LaborRate.Value * operation.WorkCenter.LaborBurdenPercent.Value * receivedQty.Value / 100;
                                }
                                #endregion
                ...
                {
                    EntityPreference en = entityPreferenceMgr.LoadEntityPreference(BusinessConstants.ENTITY_PREFERENCE_CODE_NOROUTINGTOCREATEORDER);

                    if (en != null && StringHelper.Eq(en.Value, "FALSE"))
                    {
                        throw new BusinessErrorException("Cost.Transaction.Error.OrderNotHasRouting", orderHead.OrderNo);
                    }
                }
            }
            #endregion
        ...
        public void CloseFinanceMonth(User user)
        {
            ...
            #region 成本分摊，仅按数量（工时），不能按金额分摊，因为还没有计算出成本价格
            AollcateCost(user, financeCalendar, baseCurrency, decimalLength, BusinessConstants.CODE_MASTER_COST_ALLOCATE_BY_QTY);
            #endregion
            ...
            if (prevCostBalanceList != null && prevCostBalanceList.Count > 0)
            {
                foreach (CostBalance prevCostBalance in prevCostBalanceList)
                {
                    ...
                    var intersectCostGroupAndItem = from costGroupAndItem in allCostGroupAndItems
                                                    where ((string)costGroupAndItem[0]) != currCostBalance.CostGroup.Code
                                                        && ((string)costGroupAndItem[1]) != currCostBalance.Item
            ...
            #region 成本分摊，按金额
            AollcateCost(user, financeCalendar, baseCurrency, decimalLength, BusinessConstants.CODE_MASTER_COST_ALLOCATE_BY_AMOUNT);
            #endregion
            ...
            if (invOutList != null && invOutList.Count > 0)
            {
                foreach (object[] invOut in invOutList)
                {
                    ...
                    else
                    {
                        ...
                        {
                            log.Error("扣减成本余额时，找不到零件" + invOut[0] + "，数量" + invOut[2] + "的成本明细。");
                            //throw new TechnicalException("不应该找不到本期成本明细的呀，肯定是哪里错了。生产单只有投料回冲，做了收货，没有回冲，只产生了RCT-WO事务，没有ISS-WO。");
                        }
                    }
                }
...
/*
            ...
            criteria.Add(Expression.In("TransactionType", new string[] {
                ...
                BusinessConstants.CODE_MASTER_LOCATION_TRANSACTION_TYPE_VALUE_RCT_UNP,   //盘盈
                BusinessConstants.CODE_MASTER_LOCATION_TRANSACTION_TYPE_VALUE_ISS_UNP    //盘亏
            }));
...
*/
        ...
        [Transaction(TransactionMode.Requires)]
        public void ChangeCostBalance(Dictionary<string, decimal> itemValue, string costGroupCode, User user)
        {
            return;
            foreach (var item in itemValue)
            {
                costBalanceMgr.ChangeCostBalance(item.Key, costGroupCode, item.Value, user);
            }
        }
        ...
        {
            AollcateCost(user, financeCalendar, baseCurrency, decimalLength, null);
        }

        private void AollcateCost(User user, FinanceCalendar financeCalendar, Currency baseCurrency, int decimalLength, string byAmountOrQty)
        {
            #region 取得最大生效日期
            DateTime effectiveDate = financeCalendar.EndDate.AddMilliseconds(-1);
            ...
            if (maxTransId > 0)
            {
                ...
                criteria.Add(Expression.Ge("EffectiveDate", financeCalendar.StartDate));
                criteria.Add(Expression.Lt("EffectiveDate", financeCalendar.EndDate));
                if (byAmountOrQty != null)
                {
                    criteria.Add(Expression.Eq("AllocateBy", byAmountOrQty));
                }
                ...
                if (costAllocateTransactionList != null && costAllocateTransactionList.Count > 0)
                {
                    foreach (CostAllocateTransaction costAllocateTransaction in costAllocateTransactionList)
                    {
                        ...
                        ProjectionList projectionList = Projections.ProjectionList()
                            ...
                            .Add(Projections.Sum("ActualAmount").As("ActualAmount"))
                        ...
                        foreach (object[] trans in translist)
                        {
                            ...
                            #region 增加当期成本余额
                            if (byAmountOrQty == BusinessConstants.CODE_MASTER_COST_ALLOCATE_BY_AMOUNT)
                            {
                                this.costBalanceMgr.ChangeCostBalance(costTransaction.Item, costTransaction.CostGroup.Code, costTransaction.CostElement.Code, costTransaction.ActualAmount,
                                   financeCalendar.FinanceYear, financeCalendar.FinanceMonth, user);

                                CostDetail costDetail = this.costDetailMgr.GetCostDetail(costTransaction.Item, costTransaction.CostGroup.Code, financeCalendar.FinanceYear, financeCalendar.FinanceMonth)[0];
                                if (costDetail.TotalQty != 0)
                                {
                                    costDetail.TotalAmount += costTransaction.ActualAmount;
                                    costDetail.Cost = costDetail.TotalAmount / costDetail.TotalQty;
                                    this.costDetailMgr.UpdateCostDetail(costDetail);
                                }
                            }
                            #endregion
                        }
        ...
        private void RecursionCalculateCost(FinanceCalendar financeCalendar, DateTime effectiveDate, Currency baseCurrency, User user, int decimalLength, IList<object[]> tobeCalGroupAndItems, ref IList<object[]> calculatedGroupAndItems)
        {
            ...
            foreach (object[] tobeCalGroupAndItem in tobeCalGroupAndItems)
            {
                ...
                if (pass)
                {
                    ...
                    #region 统计本期生产入库事务 + 计划外出库事务 + 盘点(盘亏/盘赢)
                    ...
                    if (tobeCalCostTransList != null && tobeCalCostTransList.Count > 0)
                    {
                        foreach (object[] tobeCalCostTrans in tobeCalCostTransList)
                        {
                            ...
                            if (itemCostInvBase != 0)  //如果库存余额等于0，成本单价等于0
                            {
                                ...
                                costDetail.TotalAmount = costBalance.Balance;
                                costDetail.TotalQty = itemCostInvBase;
                            }
                    ...
                    transInvList = this.criteriaMgr.FindAll(criteria);
