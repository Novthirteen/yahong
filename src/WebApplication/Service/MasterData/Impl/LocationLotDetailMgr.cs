...
namespace com.Sconit.Service.MasterData.Impl
{
    ...
    public class LocationLotDetailMgr : LocationLotDetailBaseMgr, ILocationLotDetailMgr
    {
        ...
        [Transaction(TransactionMode.Unspecified)]
        public decimal GetNotHuLocationLotDetailQty(string locationCode, string itemCoe)
        {
            #region 查找库存
            DetachedCriteria criteria = DetachedCriteria.For<LocationLotDetail>();
            ...
            criteria.Add(Expression.Eq("item.Code", itemCoe));

            //if (binCode != null)
            //{
            //    criteria.CreateAlias("StorageBin", "bin");
            //    criteria.Add(Expression.Eq("bin.Code", binCode));
            //}
            //else
            //{
            //    criteria.Add(Expression.IsNull("StorageBin"));
            ...
            IList invList = this.criteriaMgrE.FindAll(criteria);
            if (invList != null && invList.Count > 0 && invList[0] != null)
            {
                return (decimal)invList[0];
            }
            return 0M;
            #endregion
        }
