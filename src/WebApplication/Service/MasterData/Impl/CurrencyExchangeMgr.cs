...
namespace com.Sconit.Service.MasterData.Impl
{
    ...
    public class CurrencyExchangeMgr : CurrencyExchangeBaseMgr, ICurrencyExchangeMgr
    {
        ...
        {
            if (currencyCode.ToUpper() != baseCurrencyCode.ToUpper())
            {
                ...
                criteria.Add(Expression.Or(Expression.IsNull("EndDate"), Expression.Gt("EndDate", effDate)));
                criteria.AddOrder(Order.Desc("StartDate")); //按开始日期倒序，取最新的一个
                IList<CurrencyExchange> list = this.criteriaMgr.FindAll<CurrencyExchange>(criteria, 0, 1);
                ...
                {
                    throw new BusinessErrorException("MasterData.CurrencyExchang.NotFound", baseCurrencyCode, currencyCode, effDate.ToLongDateString());
                }
            }
            else
                return 1;
