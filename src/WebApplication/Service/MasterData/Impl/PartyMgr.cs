...
namespace com.Sconit.Service.MasterData.Impl
{
    ...
    public class PartyMgr : PartyBaseMgr, IPartyMgr
    {
        ...
        public IList<Party> GetPartys(string userCode, string moduleType)
        {

            DetachedCriteria criteria = DetachedCriteria.For<Party>();

            if (moduleType == BusinessConstants.CODE_MASTER_FLOW_TYPE_VALUE_PROCUREMENT)
            {
                DetachedCriteria[] pfCrieteria = SecurityHelper.GetSupplierPermissionCriteria(userCode);
                criteria.Add(
                    Expression.Or(
            ...
            else if (moduleType == BusinessConstants.CODE_MASTER_FLOW_TYPE_VALUE_DISTRIBUTION)
            {
                DetachedCriteria[] pfCrieteria = SecurityHelper.GetCustomerPermissionCriteria(userCode);
                criteria.Add(
                    Expression.Or(
            ...
            else if (moduleType == BusinessConstants.CODE_MASTER_FLOW_TYPE_VALUE_PRODUCTION)
            {
                DetachedCriteria[] pfCrieteria = SecurityHelper.GetRegionPermissionCriteria(userCode);
                criteria.Add(
                    Expression.Or(
