...
namespace com.Sconit.Service.MasterData.Impl
{
    ...
    public class LocationTransactionMgr : LocationTransactionBaseMgr, ILocationTransactionMgr
    {
        ...
        public void RecordLocationTransaction(OrderLocationTransaction orderLocationTransaction, InventoryTransaction inventoryTransaction, Receipt receipt, User user)
        {
            ...
            if (orderLocationTransaction.OrderDetail.OrderHead.SettleTime.HasValue)
            {
                locationTransaction.EffectiveDate = orderLocationTransaction.OrderDetail.OrderHead.SettleTime.Value;
            }
            else
            {
                locationTransaction.EffectiveDate = receipt.CreateDate;
            }
        ...
        public void RecordLocationTransaction(OrderLocationTransaction orderLocationTransaction, InventoryTransaction inventoryTransaction, InProcessLocation inProcessLocation, User user)
        {
            ...
            {
                locationTransaction.EffectiveDate = inProcessLocation.CreateDate;
            }
        ...
        public void RecordWOBackflushLocationTransaction(OrderLocationTransaction orderLocationTransaction, Item item, string huId, string lotNo, decimal qty, string ipNo, User user, Location locFrom)
        {
            ...
            locationTransaction.EffectiveDate = DateTime.Now;
            locationTransaction.RefLocation = locFrom.Code;
            locationTransaction.RefLocationName = locFrom.Name;
            locationTransaction.LocationIsFreeze = false;
            locationTransaction.ItemIsFreeze = item.Freeze;
            this.CreateLocationTransaction(locationTransaction);
        }
        ...
        public void RecordLocationTransaction(MiscOrderDetail miscOrderDetail, InventoryTransaction inventoryTransaction, User user)
        {
            ...
            locationTransaction.CreateDate = DateTime.Now;
            locationTransaction.CreateUser = user.Code;
            locationTransaction.EffectiveDate = miscOrder.EffectiveDate;
            locationTransaction.LocationIsFreeze = miscOrder.Location.Freeze;
            locationTransaction.ItemIsFreeze = miscOrderDetail.Item.Freeze;
            this.CreateLocationTransaction(locationTransaction);
        }
        ...
        {
            this.RecordLocationTransaction(inventoryTransaction, transType, user, orderNo, refLocation, productLine, receiptNo, DateTime.Now);
        }

        [Transaction(TransactionMode.Requires)]
        public void RecordLocationTransaction(InventoryTransaction inventoryTransaction, string transType, User user, string orderNo, Location refLocation, Flow productLine, string receiptNo, DateTime effTime)
        {
            LocationTransaction locationTransaction = new LocationTransaction();
            ...
            locationTransaction.CreateDate = DateTime.Now;
            locationTransaction.CreateUser = user.Code;
            locationTransaction.EffectiveDate = effTime;
            ...
            locationTransaction.ItemIsFreeze = inventoryTransaction.Item.Freeze;
            locationTransaction.LocationIsFreeze = inventoryTransaction.Location.Freeze;
            this.CreateLocationTransaction(locationTransaction);
        }
        ...
        public void RecordLocationTransaction(ProductLineInProcessLocationDetail productLineInProcessLocationDetail, string transType, User user, string ioType)
        {
            ...
            if (transType == BusinessConstants.CODE_MASTER_LOCATION_TRANSACTION_TYPE_VALUE_RCT_MATERIAL_IN)
            {
                ...
                if (ioType == BusinessConstants.IO_TYPE_OUT)
                {
                    ...
                    locationTransaction.LocationIsFreeze = productLineInProcessLocationDetail.LocationFrom.Freeze;
                }
                else
                {
            ...
            locationTransaction.CreateUser = user.Code;
            locationTransaction.EffectiveDate = DateTime.Now;
            locationTransaction.ItemIsFreeze = productLineInProcessLocationDetail.Item.Freeze;
            this.CreateLocationTransaction(locationTransaction);
        }
        ...
        public IList<LocationTransaction> GetLocationTransactionAfterEffDate(string itemCode, string loc, DateTime effDate)
        {
            ...
            criteria.Add(Expression.Gt("CreateDate", effDate));
        ...
        public IList<LocationTransaction> GetPeriodLocationTransaction(string itemCode, string loc, string transType, DateTime startDate, DateTime endDate)
        {
            ...
            criteria.Add(Expression.Le("CreateDate", endDate));
        ...
        {
            List<LocationTransaction> list = new List<LocationTransaction>();
            if (itemList != null && itemList.Count > 0)
            {
                int lotSize = 2000;
                for (int i = 0; i < itemList.Count; i = i + lotSize)
                {
                    DetachedCriteria criteria = DetachedCriteria.For(typeof(LocationTransaction));
                    ...
                    {
                        criteria.Add(Expression.Ge("CreateDate", startDate.Value));
                    }
                    var q = itemList.Skip(i).Take(lotSize).ToList();
                    criteria.Add(Expression.InG<string>("Item", q));
                    list.AddRange(criteriaMgrE.FindAll<LocationTransaction>(criteria));
                }
            }
            return list;
        }
        ...
        public IList<LocationTransaction> GetProjectionLocationTransaction(IList<string> itemList, IList<string> locList, DateTime? endDate)
        {
            ...
            if (endDate.HasValue)
                criteria.Add(Expression.Gt("CreateDate", endDate.Value));
        ...
        [Transaction(TransactionMode.Requires)]
        public override void CreateLocationTransaction(LocationTransaction entity)
        {
            if (entity.ItemIsFreeze)
            {
                throw new BusinessErrorException("MasterData.Item.Error.IsFreeze", entity.Item);
            }
            if (entity.LocationIsFreeze)
            {
                throw new BusinessErrorException("MasterData.Location.Error.IsFreeze", entity.Location);
            }
            base.CreateLocationTransaction(entity);
        }
            ...
            , User createUser)
        {
            ...
            locationTransaction.LocationIsFreeze = location != null ? location.Freeze : false;
            locationTransaction.ItemIsFreeze = item.Freeze;
