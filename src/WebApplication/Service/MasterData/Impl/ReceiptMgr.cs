...
namespace com.Sconit.Service.MasterData.Impl
{
    ...
    public class ReceiptMgr : ReceiptBaseMgr, IReceiptMgr
    {
        ...
        public void CreateReceipt(Receipt receipt, User user, bool isOddCreateHu)
        {
            ...
            if (receipt.InProcessLocations != null && receipt.InProcessLocations.Count > 0)
            {
                foreach (InProcessLocation inProcessLocation in receipt.InProcessLocations)
                {
                    ...
                    if (flowCode == null)
                    {
                        flowCode = inProcessLocation.Flow;
                    }
                    else if (flowCode != inProcessLocation.Flow)
                    {
                        throw new BusinessErrorException("Order.Error.ReceiveOrder.FlowNotEqual");
                    }
                    ...
                    SettleTime = inProcessLocation.SettleTime;
            ...
            receipt.Flow = flowCode;
            receipt.SettleTime = SettleTime;
            ...
            foreach (ReceiptDetail receiptDetail in targetReceiptDetailList)
            {
                        ...
                        && receiptDetail.HuId == null)  //如果订单设置为收货时创建Hu，但是收货时已经扫描过Hu了，按已扫描处理
                {
                        ...
                        || (receiptDetail.ScrapQty > 0))
                    {
                        ...
                        rejAndScrapReceiptDetail.ReceivedQty = 0;
                ...
                else
                {
                    ...
                    foreach (ReceiptDetail noCreateHuReceiptDetail in noCreateHuReceiptDetailList)
                    {
                        ...
                        if ((noCreateHuReceiptDetail.ReceivedQty != 0) ||
                            (noCreateHuReceiptDetail.RejectedQty != 0))
                        {
                            #region 入库
                            ...
                            #region 是否检验
                            if (orderDetail.NeedInspection && orderHead.NeedInspection
                                && inventoryTransactionList != null && inventoryTransactionList.Count > 0
                                && orderHead.SubType == BusinessConstants.CODE_MASTER_ORDER_SUB_TYPE_VALUE_NML)
                            {
                                foreach (InventoryTransaction inventoryTransaction in inventoryTransactionList)
                                {
                                    if (inventoryTransaction.Location.Type == BusinessConstants.CODE_MASTER_LOCATION_TYPE_VALUE_NORMAL
            ...
            if (receipt.OrderType != BusinessConstants.CODE_MASTER_ORDER_TYPE_VALUE_PRODUCTION &&
                receipt.InProcessLocations != null && receipt.InProcessLocations.Count > 0)
            {
                foreach (InProcessLocation inProcessLocation in receipt.InProcessLocations)
                {
        ...
        [Transaction(TransactionMode.Unspecified)]
        public IList<Receipt> GetReceipts(string IpNo)
        {
            DetachedCriteria criteria = DetachedCriteria.For(typeof(Receipt));
            criteria.Add(Expression.Eq("ReferenceIpNo", IpNo));
            IList<Receipt> receiptList = criteriaMgrE.FindAll<Receipt>(criteria);
