    ...
    public class OrderDetailMgr : OrderDetailBaseMgr, IOrderDetailMgr
    {
        ...
        public IList<OrderDetail> GenerateOrderDetail(OrderHead orderHead, FlowDetail flowDetail, bool isReferencedFlow, bool isStartKit)
        {
                ...
                || orderHead.Type == BusinessConstants.CODE_MASTER_ORDER_TYPE_VALUE_SUBCONCTRACTING)
            {
                ...
                {
                    DateTime settleTime = orderHead.SettleTime.HasValue ? orderHead.SettleTime.Value : orderHead.StartTime;
                    PriceListDetail priceListDetail = priceListDetailMgrE.GetLastestPriceListDetail(orderDetail.DefaultPriceList, orderDetail.Item, settleTime, orderHead.Currency, orderDetail.Uom);
        ...
        [Transaction(TransactionMode.Unspecified)]
        public IList<OrderDetail> GetOrderDetail(IList<OrderHead> orderHeadList)
        {
            DetachedCriteria criteria = DetachedCriteria.For<OrderDetail>();
            criteria.CreateAlias("OrderHead", "oh");
            criteria.Add(Expression.InG<OrderHead>("OrderHead", orderHeadList));

            criteria.AddOrder(Order.Asc("oh.StartDate"));
            criteria.AddOrder(Order.Asc("oh.Sequence"));
            criteria.AddOrder(Order.Asc("oh.OrderNo"));
        ...
        public bool CheckOrderDet(OrderDetail orderDetail)
        {
            ...
            else
            {
                ...
                || orderDetail.OrderHead.Type == BusinessConstants.CODE_MASTER_ORDER_TYPE_VALUE_DISTRIBUTION
                || orderDetail.OrderHead.Type == BusinessConstants.CODE_MASTER_ORDER_TYPE_VALUE_SUBCONCTRACTING)
                {
                    //检查价格单
                    EntityPreference en = entityPreferenceMgrE.LoadEntityPreference(BusinessConstants.ENTITY_PREFERENCE_CODE_NOPRICELISTCREATE);

                    if (en != null && StringHelper.Eq(en.Value, "FALSE") && !orderDetail.UnitPrice.HasValue)
                    {
                        throw new BusinessErrorException("MasterData.NoPricelist.NotAllowToCreateOrder", orderDetail.Item.Code);
                    }
                }
        ...
        public void RecordOrderShipQty(OrderLocationTransaction orderLocationTransaction, InProcessLocationDetail inProcessLocationDetail, bool checkExcceed)
        {
            ...
            try
            {
                allowPercent = decimal.Parse(entityPreference.Value);
                if (allowPercent == -1)
                {
                ...
                && orderLocationTransaction.OrderDetail.OrderHead.SubType != BusinessConstants.CODE_MASTER_ORDER_SUB_TYPE_VALUE_ADJ)
            {
                ...
                if (checkExcceed && !orderHead.AllowExceed && !allowExceedGiGR)   //不允许过量发货
                {
                    ...
                    if ((orderLocationTransaction.OrderedQty > 0 && (orderLocationTransaction.AccumulateQty + shipQty > orderLocationTransaction.OrderedQty + diffQty))
                        || (orderLocationTransaction.OrderedQty < 0 && (orderLocationTransaction.AccumulateQty + shipQty < orderLocationTransaction.OrderedQty + diffQty)))
                    {
                        throw new BusinessErrorException("Order.Error.ShipExcceed", orderHead.OrderNo, orderLocationTransaction.Item.Code);
                    }
                }
