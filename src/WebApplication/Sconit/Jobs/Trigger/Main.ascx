        ...
        <asp:GridView ID="GV_List" runat="server" AutoGenerateColumns="false" OnRowCommand="GV_List_RowCommand"
            ...
            <Columns>
                ...
                <asp:TemplateField HeaderText="${MasterData.Jobs.Trigger.PrevFireTime}">
                    <ItemTemplate>
                        <asp:Label ID="lblPrevFireTime" runat="server" Text='<%# Bind("PreviousFireTime","{0:yyyy-MM-dd HH:mm}") %>' />
                    </ItemTemplate>
                </asp:TemplateField>
                <asp:TemplateField HeaderText="${MasterData.Jobs.Trigger.NextFireTime}">
                    <ItemTemplate>
                        <asp:Label ID="lblNextFireTime" runat="server" Text='<%# Bind("NextFireTime","{0:yyyy-MM-dd HH:mm}") %>' />
                    </ItemTemplate>
                    <EditItemTemplate>
                        <asp:TextBox ID="tbNextFireTime" runat="server" onClick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm'})"
                            Text='<%# Bind("NextFireTime","{0:yyyy-MM-dd HH:mm}") %>' />
                    </EditItemTemplate>
                </asp:TemplateField>
...
<asp:Button ID="yuejie" runat="server" Text="月结" OnClick="yuejie_onclick" Visible="false"/>
<asp:Button ID="btnTest" runat="server" Text="Run BOM" OnClick="btnTest_Click" />
<asp:Button ID="btnTest1" runat="server" Text="Run Plan" OnClick="btnTest1_Click" />
