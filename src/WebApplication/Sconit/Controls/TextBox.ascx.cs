...
public partial class Controls_TextBox : System.Web.UI.UserControl, IEditableTextControl
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        //}
        this.suggest.Attributes.Add("style", "background: transparent url(Images/Icon/icon_sugg.png) no-repeat scroll 130px -2px");
    }
    ...
    private string GetWebServiceData()
    {
        if (this.ServiceMethod == "GetCacheAllItem")
        {
            return @"function Get_" + this.suggest.ClientID + @"_Data()
                {
                    $('#" + this.suggest.ClientID + @"').css('background','transparent url(Images/animated_loading.gif) no-repeat scroll 130px -2px');
                    ...
                    if (" + this.suggest.ClientID + @"_oldParameters == " + this.suggest.ClientID + @"_newParameters )
                    {
                        ...
                        $('#" + this.suggest.ClientID + @"').css('background','transparent url(Images/Icon/icon_sugg.png) no-repeat scroll 130px -2px');
                        return;
                    }

                    var para = parent.parent.topFrame.$('#data').attr('value');
                    para = eval(para);
                    //alert(para);
                    var data = {data : para};
            ...
            return @"function Get_" + this.suggest.ClientID + @"_Data()
                {
                    ...
                    $('#" + this.suggest.ClientID + @"').data('oldInputValue', " + this.suggest.ClientID + @"_inputValue);
