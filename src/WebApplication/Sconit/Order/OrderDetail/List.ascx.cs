    ...
    protected void GV_List_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        ...
        if (e.Row.RowType == DataControlRowType.DataRow)
        {
            ...
            if (orderDetail.IsBlankDetail)
            {
                ...
                if (TheOrder != null && TheOrder.LocationFrom != null)
                {
                    ((Label)e.Row.FindControl("lblLocFrom")).Visible = false;
                    DropDownList ddlLocFrom = ((DropDownList)e.Row.FindControl("ddlLocFrom"));
                    ddlLocFrom.Visible = true;
                    ddlLocFrom.SelectedValue = TheOrder.LocationFrom.Code;
                    IList<Location> locations = TheLocationMgr.GetLocationByUserCode(this.CurrentUser.Code);
                    if (locations == null)
                    {
                        locations = new List<Location>();
                    }
                    if (!locations.Select(l => l.Code).Contains(TheOrder.LocationFrom.Code))
                    {
                        Location location = TheLocationMgr.LoadLocation(TheOrder.LocationFrom.Code);
                        locations.Add(location);
                    }
                    ddlLocFrom.DataSource = locations;
                    ddlLocFrom.DataBind();
                }
        ...
        if (rfvItemCode.IsValid && rfvUom.IsValid)
        {
            ...
            if (orderDetailList != null)
            {
                ...
                for (int i = 0; i < newItemList.Count; i++)
                {
                    ...
                    if (this.NewOrEdit == "New")
                    {
                        DropDownList ddlLocFrom = (DropDownList)newOrderDetailRow.FindControl("ddlLocFrom");
                        if (ddlLocFrom.SelectedValue != string.Empty)
                        {
                            newOrderDetail.LocationFrom = TheLocationMgr.LoadLocation(ddlLocFrom.SelectedValue);
                        }
