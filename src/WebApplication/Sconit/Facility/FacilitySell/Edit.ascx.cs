...
public partial class Facility_FacilitySell_Edit : EditModuleBase
{
    ...
    protected void btnSave_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            string sellPerson = ((TextBox)(this.FV_FacilityMaster.FindControl("tbSellPerson"))).Text;
            string sellSite = ((TextBox)(this.FV_FacilityMaster.FindControl("tbSellSite"))).Text;
            string sellOrg = ((TextBox)(this.FV_FacilityMaster.FindControl("tbSellOrg"))).Text;
            ...
            facilityTrans.ToChargePerson = sellPerson;
            if (TheUserMgr.LoadUser(sellPerson) != null)
            {
                facilityTrans.ToChargePersonName = TheUserMgr.LoadUser(sellPerson).Name;
            }
            facilityTrans.ToChargeSite = sellSite;
            facilityTrans.ToOrganization = sellOrg;
            facilityTrans.TransType = FacilityConstants.CODE_MASTER_FACILITY_TRANSTYPE_SELL;
            ...
            facilityMaster.CurrChargePerson = sellPerson;
            if (TheUserMgr.LoadUser(sellPerson) != null)
            {
                facilityMaster.CurrChargePersonName = TheUserMgr.LoadUser(sellPerson).Name;
            }
            else
            {
                facilityMaster.CurrChargePersonName = string.Empty;
            }
            facilityMaster.ChargeSite = sellSite;
            facilityMaster.ChargeOrganization = sellOrg;

            TheFacilityMasterMgr.UpdateFacilityMasterAndCreateFacilityTrans(facilityMaster, facilityTrans, FacilityConstants.CODE_MASTER_FACILITY_STATUS_SELL, this.CurrentUser.Code);
            InitPageParameter(this.FCID);
            ShowSuccessMessage("Facility.FacilityMaster.FacilityMasterSell.Successfully", this.FCID);
