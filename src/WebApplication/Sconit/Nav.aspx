    ...
    <script language="javascript" type="text/javascript">
        ...
        function onTreeViewSelect(e) {
            if (e.srcElement != undefined) {
                var _src = e.srcElement.href;
                if (_src != "") {
                    var array = _src.split("Main.aspx?mid=")
                    if (array[1] != undefined) {
                        var url = "Main.aspx?mid=" + array[1];
                        var array2 = array[0].split("#");
                        if (array2[1] == undefined) {
                            e.srcElement.href = array[0] + "Nav.aspx#" + url;
                        }
                        parent.right.addTab(url, e.srcElement.innerText);
                    }
                }
            }
            else {
                var _src = e.target.href;
                if (_src != "") {
                    var array = _src.split("Main.aspx?mid=")
                    if (array[1] != undefined) {
                        var url = "Main.aspx?mid=" + array[1];
                        var array2 = array[0].split("#");
                        if (array2[1] == undefined) {
                            e.target.href = array[0] + "Nav.aspx#" + url;
                        }
                        parent.right.addTab(url, e.target.textContent);
                    }
                }
            }
        }
...
<body class="leftbody" onload="OnPageLoad()">
    ...
    <div style="position: absolute; top: 3px; left: 3px" id="DivTreeView">
        <asp:TreeView ID="TreeView1" runat="server" DataSourceID="SiteMapDataSource1" ShowLines="true"
            ExpandDepth="0" MaxDataBindDepth="2" ShowExpandCollapse="true" 
